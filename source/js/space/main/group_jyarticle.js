...
$(function(){
	...
	//getMyAttention();
	//getMyVisistor();
	getMyArticle(page_number, 5, "replace");
	getPraiseList();
...
function getArticleCategoryForManage(fromPublish){
	var temp_person_id, temp_identity_id, temp_business_type, temp_level;
	if(fromPublish == 2){
		temp_person_id = $.cookie("person_id");
		temp_identity_id = $.cookie("identity_id");
		temp_business_type = 1;
		temp_level = 2;
	}else{
		temp_person_id = org_id;
		temp_identity_id = orgidentity_id;
		temp_business_type = 2;
		temp_level = 1;
	}
	...
	$.ajax({
		...
		data:{"random_num":creatRandomNum(),
            ...
            "business_type": temp_business_type,
            "business_id": temp_person_id,
            "business_iid": temp_identity_id,
            "level": temp_level		
		},
...
function articleCategoryManage(fromPublish){
	group_person_sort = fromPublish;
	getArticleCategoryForManage(fromPublish);
	dia=art.dialog({
...
function editSortManage(type,dom,id){
    ...
    art.dialog({
        ...
        var temp_person_id, temp_identity_id, temp_level, temp_business_type;
        if(group_person_sort == 2){
        	temp_person_id = $.cookie("person_id");
        	temp_identity_id = $.cookie("identity_id");
        	temp_level = 2;
        	temp_business_type = 1;
            ...
            $.ajax({
                ...
                success : function(data) {
                    if(data.success){
                        ...
                        seq += 1;
                        getArticleCategoryForManage(group_person_sort);
...
function moveSortManage(dom,type){
    ...
    var temp_person_id, temp_identity_id;
    if(group_person_sort == 2){
    	temp_person_id = $.cookie("person_id");
    	temp_identity_id = $.cookie("identity_id");
    }else{
...
function getMyArticle(page_number, page_size, replaceOrAppend){
    ...
    $.ajax({
        ...
        url: url_path_html + "/yx/article/getServiceArticleList",
        dataType: "json",
        success: function (data) {
        	data.c_p_id = $.cookie("person_id") || "";
        	data.c_i_id = $.cookie("identity_id") || "";
        	data.is_admin = is_admin;
        	data.is_member = is_member;
        	data.bid = org_id;
        	data.biid = orgidentity_id;
        	innerHtml['data'] = data;
...
function loadMore(page_number, page_size){
	getMyArticle(+page_number+1, 5, "append");
}
	...
	art.dialog.confirm("确定要删除文章\""+Base64.decode(title)+"\"吗？", function () {
        $.ajax({
            ...
            data: {
                ...
                "business_id": org_id,
                "business_iid": orgidentity_id,
                "ids": '[{"id":'+article_id+',"b_category_id":'+category_id+',"expand_id":'+expand_id+'}]'
            },
            ...
            success: function (data) {
                if(data.success){
                    art.dialog({
                        ...
                        time: 3,
                        close: function(){
                        	getMyArticle(1, cur_page_number*5, "replace");
                            getArticleStatistics();
                            getArticleCategory();
                        }
                    });
...
function addArticles(type, article_id, expand_id){
    ...
    var html = "<div id='addArticles'>"+
        ...
        "<div class='col-md-1'>文章标题：<span style='color:red'>*</span></div>"+
        "<div class='col-md-11'>"+
        "<div id='post_title'></div>"+
        "</div>"+
        "</div>"+
        "<div class='row' style='margin:0 0 10px 0; width: 1000px;line-height: 34px;'>"+
        "<div class='col-md-1'>群组分类：</div>"+
        "<div class='col-md-2'>"+
        "<select class='form-control' id='gper_sort'></select>"+
        "</div>"+
        "<div class='col-md-1'><a href='javascript:void(0);' onclick='articleCategoryManage(3);' class='g_category_btn'>[管理]</a></div>"+
        "<div class='col-md-1'>个人分类：</div>"+
        "<div class='col-md-2'>"+
        "<select class='form-control' id='per_sort'></select>"+
        "</div>"+
        "<div class='col-md-1'><a href='javascript:void(0);' onclick='articleCategoryManage(2);' class='category_btn'>[管理]</a></div>"+
        ...
        "<div class='col-md-1'>文章内容：<span style='color:red'>*</span></div>"+
        "<div class='col-md-11'>"+
	...
	var thisdia = art.dialog({
            ...
            $.ajax({
    			...
    			success : function(data) {
    				if(data.success){
    					...
    					person_info_['roles'] = data.table_List.roles;
    					...
    					if(identity_id == 7){
    						person_info_['student_id'] = data.table_List.student_id;
    						person_info_['class_id'] = data.table_List.class_id;
    						person_info_['class_name'] = data.table_List.class_name;
    					}
    				}
		...
		//群组分类
		var gper_sort = $("#gper_sort").val();
		if(gper_sort == 0 || !!!gper_sort){
			dialogOk("请选择群组分类！",300,"");
			return false;
		}
        //个人分类
        ...
        ue.getKfContent(function(content) {
            $.ajax({
                ...
                url: url_path_action + (type == 1 ? "/blog_expand/update_article_expand" : "/blog_expand/save_article_expand"),
                dataType: "json",
                success: function (data) {
                    if (data.success) {
                        art.dialog({
                            ...
                            init: function () {
                                ...
                                if (type != 1) {
                                	...
                                	if(identity_id == 106){
                                		bbbid = 151;
                                	}else if(identity_id == 107){
                                		bbbid = 181;
                                	}
                                    var options = {
                                        business_type:106,
                                        relatived_id:data.id,
                                        r_person_id:"",
                                        r_identity_id:"",
                                        operation_content:$.cookie("person_name") + '发表了文章' + post_title + '。',
                                        e_relatived_id:data.expand_id
                                    };
                                    creditWriteToQueue(options);
                                    options.business_type = bbbid;
                                    options.r_person_id = org_id;
                                    options.r_identity_id = orgidentity_id;
                                    creditWriteToQueue(options);
...
function getArticleInfo(article_id, expand_id){
    ...
    $.ajax({
        ...
        url: url_path_action_login + "/blog_expand/get_article_expand",
        dataType: "json",
        success: function (data) {
            if(data.success){
                $("#per_sort").val(data.person_category_id);
                $("#gper_sort").val(data.b_category_id);
...
//个人选取文章
function checkPersonArticle(){

    art.dialog.open(url_path_html + '/yx/html/manage/article/showPersonArticle.html?id='+org_id+"&iid=106", {
        title: '选取个人文章',
        height: 500,
        width: 900,
        lock:true,
...
var group_id = "";
	...
	var url = url_path_html + "/yx/html/manage/article/verifyArticleList4workroom.html?group_id=" + org_id;
	
	var dialog = art.dialog.open(url, {
		id : "toVerifyArticleList4workroom",
		lock : true,
		title : '文章审核',
		width : "80%",
		height : "60%",
		close : function() {
			getMyArticle(1, 5, "replace");
			getArticleStatistics();// 审核之后刷新统计数据
			getArticleCategory();
		}
	});
}