var Gallery = function(){
	var person_id,identity_id,visit_to,page_number = 1,business_type="1",
	    page_size = 20,
	    total_page = 1,
	    cur_category = "",
	    total_row;
	...
	/*上传图片点击事件*/
	function doupload(){
		tb_show("上传照片","upload_photo.html?folderid="+ cur_category+"&id="+person_id+"&identity_id="+identity_id+"&business_type="+business_type+"&visit_to="+visit_to+"&TB_iframe=true&height=380&width=700","thickbox");
	}
	    ...
	    $.ajax({
	        ...
	        success : function(data) {
	            if(data.success){
	                ...
	                $(dom).removeAttr("onclick","cancelPraise("+pictureId+",this)");
	...
	function start(){
		$.get("../tpl/main/main_gallery_tpl.html", function(data){
			//1.替换main div内容
			$("#main").html(data);
			//2.加载css，然后回调
			loadScripts(new Array("../../../css/space/space_gallery.css"), 0, function(){
			    if((GetQueryString("id")!=null)&&(GetQueryString("identity_id")!=null)){
			        ...
			        if($.cookie("person_id") == null){
			            ...
			            visit_to = ($.cookie("person_id") == person_id && $.cookie("identity_id") == identity_id) ? 3 : 2;
			        }
			    ...
			    var innerHtml={};
			    innerHtml['visit_to']=visit_to;
			    $("#mainGallery").html(template('mainGallery_inner', innerHtml));
	...
	var outer = {
		...
		galleryCategoryManage: galleryCategoryManage,
		//getGalleryCategoryForManage: getGalleryCategoryForManage,
		...
		//scrollSetTimeout: scrollSetTimeout,
		comment: comment,
		addPraise: addPraise,
		//cancelPraise: cancelPraise,
		addDownLoadNum: addDownLoadNum,
		//addBrowseNum: addBrowseNum,
		//reloadPictureLi: reloadPictureLi,
		//reloadPicturePraise: reloadPicturePraise,
		editPhoto: editPhoto,
