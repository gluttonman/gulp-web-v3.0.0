...
//1、默认白名单为论坛或者问答等情况使用，使用方法filterXSS(html)

//2、2014-11-26添加自定义规则，为文本框输入过滤xss（过滤所有标签），使用方法：myxss.process(html);
var options_input = 
{
	whiteList: 
	{
...
myxss = new filterXSS.FilterXSS(options_input);

//3、2014-11-27添加自定义规则，为文本框输入过滤xss（不过滤br标签），使用方法：myxss2.process(html);
var options_input2 = 
{
	whiteList: 
	{
...
myxss2 = new filterXSS.FilterXSS(options_input2);